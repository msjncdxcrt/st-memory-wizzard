...
> **English abstract** — A zero-dependency local reverse proxy for SillyTavern. It rewrites the plain-text marker `[[CACHE_BREAK]]` (which survives ST's `convertClaudeMessages` stripping) back into real Anthropic `cache_control` blocks at the HTTP layer, so native Claude sources can actually hit the prompt cache. Optional — only needed for the gateway cache mode. See the Chinese guide below for setup.
